...
<template>
  <div class="calculator p-4 max-w-md mx-auto">
    ...
    <input
id="result" v-model="displayValue"
      class="font-mono border border-gray-300 rounded p-2 mb-4 w-full bg-green-700 text-black" type="text" disabled />
    <div class="grid grid-cols-5 gap-2">
      <button
v-for="(button, index) in buttons.filter((b) => !b.fullWidth)" :key="`${button.label}-${index}`"
        :class="button.style" @click="button.action">
        {{ button.label }}
      </button>
      <!-- Equals button spanning full width -->
      <button
v-for="(button, index) in buttons.filter((b) => b.fullWidth)" :key="`${button.label}-fullwidth-${index}`"
        :class="button.style + ' col-span-5'" @click="button.action">
        {{ button.label }}
      </button>
    </div>
  </div>

...
import { CALCULATOR_BUTTONS, createButtonHandler, DEFAULT_STYLE_CLASSES } from './button-config.js';
...
describe('Calculator Button Configuration', () => {
  ...
  describe('CALCULATOR_BUTTONS configuration', () => {
    ...
    it('should have exactly one equals button with fullWidth property', () => {
      const equalsButtons = CALCULATOR_BUTTONS.filter(button => button.fullWidth);
    ...
    it('should have all digit buttons 0-9', () => {
      const digitButtons = CALCULATOR_BUTTONS.filter(button => 
        button.actionType === 'digit'
      );
      expect(digitButtons).toHaveLength(10);
      
      for (let i = 0; i <= 9; i++) {
        const digitButton = digitButtons.find(button => button.actionData === i);
    ...
    it('should have decimal point button', () => {
      const decimalButton = CALCULATOR_BUTTONS.find(button => button.label === '.');
      expect(decimalButton).toBeDefined();
      expect(decimalButton?.actionType).toBe('decimal');
    });

    it('should have clear buttons', () => {
      const clearButton = CALCULATOR_BUTTONS.find(button => button.label === 'C');
      const clearEntryButton = CALCULATOR_BUTTONS.find(button => button.label === 'CE');
  ...
  describe('createButtonHandler', () => {
    it('should handle digit buttons correctly', () => {
      const digitButton = CALCULATOR_BUTTONS.find(button => button.label === '7')!;
      const handler = createButtonHandler(digitButton, calculator, updateDisplay);
    ...
    it('should handle decimal button correctly', () => {
      const decimalButton = CALCULATOR_BUTTONS.find(button => button.label === '.')!;
      const handler = createButtonHandler(decimalButton, calculator, updateDisplay);
    ...
    it('should handle operation buttons correctly', () => {
      ...
      const addButton = CALCULATOR_BUTTONS.find(button => button.label === '+')!;
    ...
    it('should handle unary operation buttons correctly', () => {
      ...
      const sqrtButton = CALCULATOR_BUTTONS.find(button => button.label === 'sq')!;
      const handler = createButtonHandler(sqrtButton, calculator, updateDisplay);
    ...
    it('should handle memory buttons correctly', () => {
      ...
      const memoryAddButton = CALCULATOR_BUTTONS.find(button => button.label === 'M+')!;
      const memoryRecallButton = CALCULATOR_BUTTONS.find(button => button.label === 'MR')!;
      
      const addHandler = createButtonHandler(memoryAddButton, calculator, updateDisplay);
      const recallHandler = createButtonHandler(memoryRecallButton, calculator, updateDisplay);
    ...
    it('should handle clear buttons correctly', () => {
      ...
      const clearButton = CALCULATOR_BUTTONS.find(button => button.label === 'C')!;
      const handler = createButtonHandler(clearButton, calculator, updateDisplay);
    ...
    it('should handle calculate button correctly', () => {
      ...
      const equalsButton = CALCULATOR_BUTTONS.find(button => button.label === '=')!;
      const handler = createButtonHandler(equalsButton, calculator, updateDisplay);

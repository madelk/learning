  ...
  // XO-inspired base configuration with comprehensive plugin setup
  {
    ...
    languageOptions: {
      globals: {
        ...
        ...Object.fromEntries(
          Object.entries(globals.browser).filter(([key]) => key !== 'AudioWorkletGlobalScope ')
        ),
    ...
    settings: {
      ...
      'import/resolver': {
        ...
        typescript: {
          alwaysTryTypes: true,
          project: ['tsconfig.json', 'tsconfig.*.json', './*/tsconfig.json', './*/tsconfig.*.json']
        }
      }
    },
    rules: {
      ...
      'n/file-extension-in-import': ['error', 'always', { '.ts': 'never', '.tsx': 'never' }],
